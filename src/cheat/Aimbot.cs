            ...
            while (true)
            {
                if (Logic.isAimbotEnabled)
                {
                    if ((SDK.OsuManager.ProcessManager.ClientType == ClientTypes.Stable && !Config.config.overrides.overrideaimbot) 
                        || (SDK.OsuManager.ProcessManager.ClientType == ClientTypes.Lazer && !Config.config.overrides.overrideaimbot))
                    {
                        new Stable(SDK).Loop();
                    }
                    else if ((SDK.OsuManager.ProcessManager.ClientType == ClientTypes.Lazer && !Config.config.overrides.overrideaimbot) 
